...
if len(API_HASH) == 0:
    ...
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '7653138055:AAH2Gg4kfBvNrv_j5kSlKlhdqGTmXBspAQs') #bot token from botfather
if len(BOT_TOKEN) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1002395727504').split()]
if len(INDEX_CHANNELS) == 0:
    print('Info - INDEX_CHANNELS is empty')
AUTH_CHANNEL = [int(auth_channels) for auth_channels in environ.get('AUTH_CHANNEL', '-1002398866124').split()]
if len(AUTH_CHANNEL) == 0:
    print('Info - AUTH_CHANNEL is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1002495227151') #bot log channel -1005293546253
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://infohubstore06:nGQnHQt2QXvQyM7E@hollyhell.etgk7.mongodb.net/?retryWrites=true&w=majority&appName=hollyhell") #mongo db url
if len(DATABASE_URL) == 0:
    print('Error - DATABASE_URL is missing, exiting now')
    exit()
DATABASE_NAME = environ.get('DATABASE_NAME', "hollyhell")
...
INDEX_EXTENSIONS = [extensions.lower() for extensions in environ.get('INDEX_EXTENSIONS', 'pdf epub mobi mp3 m4b aac wav').split()]
STICKERS_IDS = (
